...
export function AssignEpicIssueModal({ isOpen, onClose, onAssign, currentIssueIds = [] }) {
  const [selectedIds, setSelectedIds] = useState([])
  const [searchInput, setSearchInput] = useState('')
  const [submitting, setSubmitting] = useState(false)

  // Only fetch issues without an epic when modal is open (pass null to skip)
  const { issues: availableIssues, loading } = useIssues(isOpen ? { epic: 'none' } : null)

  // Filter out issues already in this epic and apply search
  const filteredIssues = useMemo(() => {
    if (!availableIssues || !isOpen) return []
    const available = currentIssueIds.length > 0
      ? availableIssues.filter(issue => !currentIssueIds.includes(issue.id))
      : availableIssues
  ...
  }, [availableIssues, searchInput, currentIssueIds, isOpen])
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title="Agregar Issues a la Epica"
      description="Selecciona los issues sin epica para asignar a esta epica"
      size="lg"
    >
            ...
            <div className="py-8 text-center">
              <p className="text-text-muted text-sm">
                {searchInput
                  ...
                  : 'No hay issues disponibles sin epica'}
              </p>
            </div>
          ) : (
            <AnimatePresence>
